...
std::unique_ptr<SearchStopper> AlphazeroTimeManager::GetStopper(
  ...
  if (!alphazero_modified_) {
    ...
    new_alphazerotimepct_ = std::min<float>(100 , (alphazerotimepct_ * (tuned_initial_time_ / *time ) *
                            (expected_game_time_ / expected_tuned_game_time_)));
    alphazero_decay_ =
        (1 / expected_moves_) * (new_alphazerotimepct_ - alphazerotimepct_);
  ...
  // Using incrementally more of the increment based on the difference between initial total time and current total time.
